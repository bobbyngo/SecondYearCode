...
{ 
	...
	if (f.num == 0) {
		...
		f.num = f.num / commonDiv;
 		f.den = f.den / commonDiv;
	}
...
fraction_t add_fractions(fraction_t f1, fraction_t f2)
{
	...
	sumf.num = (f1.num * f2.den) + (f2.num * f1.den);
	sumf.den = f1.den * f2.den ;


	return make_fraction(sumf.num, sumf.den);
}
...
{
	fraction_t productf = {0,0};

	productf.num = f1.num * f2.num;
	productf.den = f1.den * f2.den;

	return make_fraction(productf.num, productf.den);
}
